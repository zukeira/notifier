...
<body>
  <div class="menu">
    ...
    <a href="adm.html" class="icon"><i class="fas fa-user"></i></a>
  </div>
  ...
  if (strlen($user_name) || strlen($user_cellphone)) {
    if ($user_name === "admin" && $user_cellphone === "123") {
        // Credenciais corretas, redirecionar para adm.php
        header("Location: adm.php");
        exit();
    } else {
        // Outras ações quando as credenciais não correspondem (pode exibir uma mensagem de erro)
    }
?>
    ...
    <div class= "container">
      <div class="center">
        ...
        <form action="<?PHP echo $_SERVER['SCRIPT_NAME'] ?>" method="POST">
          <table border="0">
            <div class="txt_field">
              <input type="text" name="NAME"  id= "iduser" required>
              <span></span>
              <label>Nome completo
              </label>
            </div>
            <div class="txt_field">
              <input type="text" name="CELLPHONE" id= "user" required>
              <span></span>
              <label>CPF</label>
            </div>
